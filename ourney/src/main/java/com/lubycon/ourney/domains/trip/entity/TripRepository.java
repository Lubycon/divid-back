...
public interface TripRepository extends JpaRepository<Trip, UUID> {
    @Query("SELECT T.tripId FROM Trip T WHERE T.tripName = :tripName AND T.tripIndex = :tripIndex AND T.ownerId = :ownerId")
    UUID findIdByTripName(@Param("tripName") String tripName, @Param("tripIndex") long tripIndex, @Param("ownerId") long ownerId);
    ...
    @Query("SELECT COUNT(t.tripId) FROM Trip t")
    long countAllByTripId();
